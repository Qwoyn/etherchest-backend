...
hivejs.api.getAccountHistory(null, null, username, -1, 100, function(err, result) {
  if (err){
    console.log(err)
    startWith(sh)
  } else {
  ...
  if(state.cs == null) {
    ...
    processor.onBlock(function(num, block) {
        ...
        if (num % 5 === 0 && processor.isStreaming()) {
            ...
            let gemPrice = price * 1000;
            
            // sets state to gem price + 2 percent and 30 HIVE to make up for price difference
            state.stats.prices.listed.gems.diamond = Math.ceil(gemPrice * 1.03) + 9000;
            state.stats.prices.listed.gems.sapphire = Math.ceil((gemPrice * 1.03) / 2) + 9000;
            state.stats.prices.listed.gems.emerald = Math.ceil((gemPrice * 1.03) / 4) + 9000;
            state.stats.prices.listed.gems.ruby = Math.ceil((gemPrice * 1.03) / 10) + 9000;
